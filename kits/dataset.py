...
def get_dataset(
    ...
    if not os.path.exists(splits_path):
        splits_path = "kits/splits"
    
    def get_files(sample_names):
        img_list = []
        seg_list = []
        for sample_name in sample_names:
            # Only process if sample_name is a valid case directory
            case_dir = os.path.join(data_path, sample_name)
            if not os.path.isdir(case_dir):
                continue
            img_files = sorted([f for f in os.listdir(case_dir) if f.startswith('img-') and f.endswith('.jpg')])
            seg_files = sorted([f for f in os.listdir(case_dir) if f.startswith('seg-') and f.endswith('.png')])
            img_list += [os.path.join(case_dir, f) for f in img_files]
            seg_list += [os.path.join(case_dir, f) for f in seg_files]
...
class KITSSegDatasetWithConf(Dataset):
...
def get_seg_dataset_with_conf(data_path: str, lab_path: str, conf_path: str = None) -> tuple[Dataset, Dataset, Dataset]:
    ...
    def get_files(sample_names):
        ...
        for sample_name in sample_names:
            case_dir = os.path.join(data_path, sample_name)
            num_files = len(os.listdir(case_dir)) // 2
            img_list += [
                os.path.join(case_dir, f"img-{str(i).zfill(3)}.jpg")
                for i in range(num_files)
            ]
            seg_list += [
                os.path.join(case_dir, f"seg-{str(i).zfill(3)}.png")
                for i in range(num_files)
            ]
        return img_list, seg_list

    train_dataset = KITSSegDatasetWithConf(
        *get_files(
            sample_name.strip() for sample_name in open(os.path.join(splits_path, "train.txt"), "r")
        ),
        lab_path,
    ...
    val_dataset = KITSSegDatasetWithConf(
        *get_files(
            sample_name.strip() for sample_name in open(os.path.join(splits_path, "val.txt"), "r")
        ),
        lab_path,
    ...
    test_dataset = KITSSegDatasetWithConf(
        *get_files(
            sample_name.strip() for sample_name in open(os.path.join(splits_path, "test.txt"), "r")
        ),
        lab_path,
