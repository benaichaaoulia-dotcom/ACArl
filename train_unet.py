...
def main():
    ...
    for n_iter in pbar:
        ...
        imgs = datapack["img"].cuda().float()
        segs = datapack["seg"].cuda()
        labs = datapack["lab"].float().cuda()

        with autocast():
            x = model(imgs)
        
        x = x.float()
        weight = torch.tensor([10.0, 1.0], dtype=torch.float32, device=imgs.device)
        loss = cross_entropy(x, labs, weight=weight)
        ...
        score_list, cnt = dice(pred[:, 1:], segs)
        score = np.mean(score_list) if score_list else 0.0
